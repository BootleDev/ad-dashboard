...
export default function ABSignificance({ mergedAds }: Props) {
  const results = useMemo(() => {
    ...
    for (const [, ads] of Object.entries(groups)) {
      ...
      if (impA < 100 || impB < 100) continue; // Need minimum sample
    ...
    >
      ...
      {results.length === 0 ? (
        <p className="text-xs" style={{ color: "var(--text-secondary)" }}>
          No A/B variant pairs detected with enough data (need 100+ impressions each). Name variants with suffixes like &quot;- A&quot;, &quot;- B&quot; for auto-detection.
        </p>
      ) : (
            ...
            >
              ...
              <div className="flex gap-4 text-xs">
                <div className={test.winner === "A" ? "text-green-400" : ""}>
                  <span className="font-medium">A:</span> {test.adA} ({test.valueA.toFixed(2)}%)
                </div>
                <div className={test.winner === "B" ? "text-green-400" : ""}>
                  <span className="font-medium">B:</span> {test.adB} ({test.valueB.toFixed(2)}%)
                </div>
              </div>
            </div>
          ))}
